...
class VR_algorithm():
    ...
    def SVRG_step(self, ite, **kwargs):
        ...
        local_sample_size_max = kwargs.get('local_sample_size_max',self.cost_model.N)
        N_by_batch = int(np.floor(self.cost_model.N / minibatch))   # how many 
        epoch_per_FG = kwargs.get("epoch_per_FG", 2) # particular for SVRG
        using_sgd = kwargs.get('using_sgd',1)

        if ite % (epoch_per_FG*local_sample_size_max) == 0:
            self.reorder =  np.random.choice(N_by_batch, epoch_per_FG*local_sample_size_max, replace=kwargs.get('replace', True))
            self.w_at_start = copy.deepcopy(self.cost_model.w)
            self.grad_full_at_start = self.cost_model.full_gradient()

        idx = self.reorder[ite % (epoch_per_FG*local_sample_size_max)]
...
class Dist_VR_agent_directed(Dist_VR_agent):
    def __init__(self, X, y, w_star, cost_model, **kwargs):
        Dist_VR_agent.__init__(self, X, y, w_star, cost_model, **kwargs)

        # typical for directed exact diffusion
        self.id = uuid.uuid4()

        self.phi = np.zeros((self.M,1))
        self.psi = np.zeros((self.M,1))
        self.psi_last = np.zeros((self.M,1)) # used for exact diffusion
        self.p = {self.id: 1}      # local perron eigenvector, initialized as 1
        self.p_tmp = {self.id: 1}
        # self.mu = mu    # local step-size

    def adapt(self, mu, ite, method='GD', style='Diffusion', **kwargs):
        self.p = self.p_tmp.copy()  # sync the old p

        Dist_VR_agent.adapt(self, mu/self.p[self.id], ite, method=method, style=style, **kwargs)


    def combine(self, weight_list, w_list, p_list, style='Diffusion'):
        if len(weight_list) != len(w_list) or len(weight_list) != len(p_list) or len(p_list) != len(w_list):
            print ("need match!")
            raise

        # calculate w (Diffusion) or phi (EXTRA)
        w_tmp = np.zeros( (self.M,1) )
        for i, weight in enumerate(weight_list):
            w_tmp += weight*w_list[i]
        ...
        # calculate p (naive implementation)
        self.p_tmp = {}
        for a, p in zip(weight_list, p_list): 
            for id in p.keys():
                self.p_tmp[id] = self.p_tmp.get(id, 0) + a*p[id]


    def get_p(self):
        print (self.p[self.id])
...
class ZMQ_VR_agent(multi_VR_agent_self):
    ...
    def __init__(self, X, y, w_star, cost_model, socket=None, **kwargs):
        ...
        if socket is None:
            ...
            self.socket = socket        # self.socket is a dictionary {nbr_idx: nbr_socket}
    ...
    def combine(self, ite, style='Difffusion'):
        ...
        if len(self.socket) == 0:   # self update
            if style == 'Diffusion':
                self.cost_model.w = self.phi.copy()
            elif style == 'EXTRA':
                self.phi = self.cost_model.w.copy()
        ...
        if style == 'Diffusion': 
            x = {'name': self.name, 'val': self.phi.tolist()}
        elif style == 'EXTRA': 
            x = {'val': self.cost_model.w.tolist()}


        # Send variable to each neighbor
        for idx, nbr_socket in self.socket.items():
            nbr_socket[0].send_json(x)
        
        # receive variables from each neighbor
        tmp = self.weights[0] * self.phi if style == 'Diffusion' else self.weights[0] * self.cost_model.w
        for idx, nbr_socket in self.socket.items():
            recv_x = nbr_socket[1].recv_json()
            tmp += self.weights[idx] * np.array(recv_x[u'val'])
...
#         if style == 'Diffusion': 
#             x = {'name': self.name, 'val': self.phi.tolist(), 'neighbor': self.neighbor}
#         elif style == 'EXTRA': 
...
#         # receive variables from each neighbor
#         tmp = self.phi/(self.neighbor + 1) if style == 'Diffusion' else self.cost_model.w/(self.neighbor + 1)
#         for nb in range(self.neighbor):
#             recv_x = self.socket[nb].recv_json()
#             tmp += np.array(recv_x[u'val'])/(self.neighbor + 1)
